...
osg::Geometry* SurfaceObject::drawGeometry() const
{
  ...
  constexpr bool degenerate = true; // Degenerate triangles
  constexpr bool restart = false; // Primitive restart index
  constexpr itype r = 0; // Restart index
  constexpr itype o = restart ? r + 1 : 0; // Index offset
  ...
  /* Debug */
#if 1 // Visualize normals
  osg::ref_ptr<Vec3Array> facetsCenters = new Vec3Array();
  osg::ref_ptr<Vec3Array> facetsNormals = new Vec3Array();
  ...
  if (normalized) {
...
#if 0 // Ragged array (with allocator overhead)
#else // Contiguous array (with pointers overhead)
    Vec3::value_type**** A = Au;
#endif

#if 1 // Weighted by area and/or angle
#if 0 // Area-weighted
    constexpr itype na = 3/*1*/; // FIXME: It is simpler to create an array of three weights per facet (equal, area, angle) because it avoids too many checks when filling the array and it would be symmetric with the array of adajacent facets normals A => merge A and W in a greater array of two elts in first dim
#elif 0 // Angle-weighted
    constexpr itype na = 3/*1*/; // FIXME
#elif 1 // Weighted by area and angle
    constexpr itype na = 3/*2*/; // FIXME
#endif

#if 0 // Ragged array (with allocator overhead)
    Vec3::value_type**** W = nullptr;
#else // Contiguous array (with pointers overhead)
    ...
    for (itype u = 0; u < nu; u++) {
      if (X[u][nvt0] == X[u][nvm1] &&
          Y[u][nvt0] == Y[u][nvm1] &&
          Z[u][nvt0] == Z[u][nvm1]) {
        A[u][nvm1][i][x] = A[u][nvt0][i][x];
        A[u][nvm1][i][y] = A[u][nvt0][i][y];
        A[u][nvm1][i][z] = A[u][nvt0][i][z];
        A[u][nvm1][l][x] = A[u][nvt0][l][x];
        A[u][nvm1][l][y] = A[u][nvt0][l][y];
        A[u][nvm1][l][z] = A[u][nvt0][l][z];
        A[u][nvm1][j][x] = A[u][nvt0][j][x];
        A[u][nvm1][j][y] = A[u][nvt0][j][y];
        A[u][nvm1][j][z] = A[u][nvt0][j][z];
        A[u][nvt0][m][x] = A[u][nvm1][m][x];
        A[u][nvt0][m][y] = A[u][nvm1][m][y];
        A[u][nvt0][m][z] = A[u][nvm1][m][z];
        A[u][nvt0][k][x] = A[u][nvm1][k][x];
        A[u][nvt0][k][y] = A[u][nvm1][k][y];
        A[u][nvt0][k][z] = A[u][nvm1][k][z];
        A[u][nvt0][n][x] = A[u][nvm1][n][x];
        A[u][nvt0][n][y] = A[u][nvm1][n][y];
        A[u][nvt0][n][z] = A[u][nvm1][n][z];
#if 1 // Weighted by area and/or angle
        W[u][nvm1][i][0] = W[u][nvt0][i][0];
        W[u][nvm1][i][1] = W[u][nvt0][i][1];
        W[u][nvm1][l][0] = W[u][nvt0][l][0];
        W[u][nvm1][l][1] = W[u][nvt0][l][1];
        W[u][nvm1][j][0] = W[u][nvt0][j][0];
        W[u][nvm1][j][1] = W[u][nvt0][j][1];
        W[u][nvt0][m][0] = W[u][nvm1][m][0];
        W[u][nvt0][m][1] = W[u][nvm1][m][1];
        W[u][nvt0][k][0] = W[u][nvm1][k][0];
        W[u][nvt0][k][1] = W[u][nvm1][k][1];
        W[u][nvt0][n][0] = W[u][nvm1][n][0];
        W[u][nvt0][n][1] = W[u][nvm1][n][1];
#endif
      }
    }
    for (itype v = 0; v < nv; v++) {
      if (X[nut0][v] == X[num1][v] &&
          Y[nut0][v] == Y[num1][v] &&
          Z[nut0][v] == Z[num1][v]) {
        A[num1][v][i][x] = A[nut0][v][i][x];
        A[num1][v][i][y] = A[nut0][v][i][y];
        A[num1][v][i][z] = A[nut0][v][i][z];
        A[num1][v][k][x] = A[nut0][v][k][x];
        A[num1][v][k][y] = A[nut0][v][k][y];
        A[num1][v][k][z] = A[nut0][v][k][z];
        A[num1][v][n][x] = A[nut0][v][n][x];
        A[num1][v][n][y] = A[nut0][v][n][y];
        A[num1][v][n][z] = A[nut0][v][n][z];
        A[nut0][v][l][x] = A[num1][v][l][x];
        A[nut0][v][l][y] = A[num1][v][l][y];
        A[nut0][v][l][z] = A[num1][v][l][z];
        A[nut0][v][j][x] = A[num1][v][j][x];
        A[nut0][v][j][y] = A[num1][v][j][y];
        A[nut0][v][j][z] = A[num1][v][j][z];
        A[nut0][v][m][x] = A[num1][v][m][x];
        A[nut0][v][m][y] = A[num1][v][m][y];
        A[nut0][v][m][z] = A[num1][v][m][z];
#if 1 // Weighted by area and/or angle
        W[num1][v][i][0] = W[nut0][v][i][0];
        W[num1][v][i][1] = W[nut0][v][i][1];
        W[num1][v][k][0] = W[nut0][v][k][0];
        W[num1][v][k][1] = W[nut0][v][k][1];
        W[num1][v][n][0] = W[nut0][v][n][0];
        W[num1][v][n][1] = W[nut0][v][n][1];
        W[nut0][v][l][0] = W[num1][v][l][0];
        W[nut0][v][l][1] = W[num1][v][l][1];
        W[nut0][v][j][0] = W[num1][v][j][0];
        W[nut0][v][j][1] = W[num1][v][j][1];
        W[nut0][v][m][0] = W[num1][v][m][0];
        W[nut0][v][m][1] = W[num1][v][m][1];
#endif
      }
    }
#endif
    ...
    // TODO average
#if 0 // No average (per-primitive normals)
    ...
    for (itype u = 0; u < nu; u++) {
      for (itype v = 0; v < nv; v++) {
        ...
        for (itype w = 0; w < nw; w++) {
          // FIXME simplify below => have a single constant weight = 1 (* area) (* angle)
#if 1 // Weighted by area and/or angle
#if 0 // Area-weighted
          normal[x] += A[u][v][w][x] * W[u][v][w][0];
          normal[y] += A[u][v][w][y] * W[u][v][w][0];
          normal[z] += A[u][v][w][z] * W[u][v][w][0];
#elif 0 // Angle-weighted
          normal[x] += A[u][v][w][x] * W[u][v][w][1];
          normal[y] += A[u][v][w][y] * W[u][v][w][1];
          normal[z] += A[u][v][w][z] * W[u][v][w][1];
#elif 1 // Weighted by area and angle
          normal[x] += A[u][v][w][x] * W[u][v][w][0] * W[u][v][w][1];
          normal[y] += A[u][v][w][y] * W[u][v][w][0] * W[u][v][w][1];
          normal[z] += A[u][v][w][z] * W[u][v][w][0] * W[u][v][w][1];
#endif
#else // Equal weights
          normal[x] += A[u][v][w][x];
          normal[y] += A[u][v][w][y];
